...
/**
@author fxa579
 Base class for Vector mathematics in 2-Dimensions
 */
public class Vector2 implements Serializable {
    ...
    public float magnitude(Vector2 vector) {
        float a = 0.96f, b = 0.4f, ax = Math.abs(vector.getX() - getX()), by = Math.abs(vector.getY() - getY());
    ...
    public float exactMagnitude(Vector2 vector) {
        return (float)Math.sqrt(Math.pow(vector.getX() - getX(), 2) + Math.pow(vector.getY() - getY(), 2));
    }
