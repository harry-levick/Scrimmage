...
/**
 * Linked list implementation of the Deque interface (not available in Java 5, so not formally
 * declared) where the link pointers are tightly integrated with the element. Linked deques have no
 * capacity restrictions; they grow as necessary to support usage. They are not thread-safe; in the
 * absence of external synchronization, they do not support concurrent access by multiple threads.
 * Null elements are prohibited.
 * <p>
 * Most <tt>LinkedDeque</tt> operations run in constant time by assuming that the {@link Linked}
